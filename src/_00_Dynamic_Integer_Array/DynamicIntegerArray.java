...
private int[] integers;
	...
	public void add(int v) {
		...
		int[] integersAdd = new int[integers.length+1];
		//B. set the last element of your new array to 
		//   the value passed into the method
		integersAdd[integersAdd.length-1] = v;
		//C. iterate through the member array and 
		//   copy every element from the member array 
		//   to the new array
		for (int i = 0; i < integers.length; i++) {
			integersAdd[i] = integers[i];
		}
		//D. set the member array equal to the new array.
		integers = integersAdd;
	}
	...
	public void insert(int v, int location) {
		//A. create and initialize a new int array to be one 
		//   element longer than the member array
		int[] integersInsert = new int[integers.length+1];
		//B. Make a for loop that iterates through the new array
for (int i = 0; i < integersInsert.length; i++) {
	if(i < location) {
		integersInsert[i] = integers[i];
	}else if(i > location) {
		integersInsert[i] = integers[i-1];
	}else {
		integersInsert[i] = v;
	}
}
	...
	public void remove(int location) {
		...
		int[] integersRemove = new int[integers.length-1];
		//B. make a for loop to iterate through the member array
		for (int i = 0; i < integers.length; i++) {
			if(i < location) {
				integersRemove[i] = integers[i];
			}else if(i > location) {
				integersRemove[i-1] = integers[i];
			}
		}
			//C. if i  is less than location
			//		set the element at i of the new array to the element at i of the member array
